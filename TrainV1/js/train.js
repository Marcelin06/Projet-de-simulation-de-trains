 ...
 */
class Type_de_train{
	static Locomotive		= new Type_de_train('locomotive');
...
/*varibale globale
*Le tableau contient 8 cases booleennes qui permettent de savoir 
*si un bouton qui n'est pas un train est selectionne
*incice 0 pour boutton Foret F
*incice 1 pour boutton Eau E
*incice 2 pour boutton Rail_horizontal =
*incice 3 pour boutton Rail_vertical ||
*incice 4 pour boutton Rail_droite_vers_haut 
*incice 5 pour boutton Rail_haut_vers_droite
*incice 6 pour boutton Rail_droite_vers_bas
*incice 7 pour boutton Rail_bas_vers_droite	
*/
let tab_bool_bouton = [false, false, false, false, false, false, false, false];
...
function image_of_case(type_de_case){
	switch(type_de_case){
		...
		case Type_de_train.Locomotive			: return IMAGE_LOCO;
	...
	switch (type_train) {
		case Type_de_train.Train1:
			voiture1 = {
				x: colone_case,
				y: ligne_case,
				type_: Type_de_train.Locomotive
			};
...
//Placement d'une locomotive ou d'un wagon dans le paleau aux coordonnees indiquees et dessin de la locomotive ou du wagon
...
function dessinerTrain(le_train, plateau, contexte){
	...
	for(i = 0; i < le_train.tableau.length; i++){
		if(0 === i){
			placerVoitureSurPlateau(plateau, le_train.tableau[i].y, le_train.tableau[i].x, contexte, Type_de_train.Locomotive);
...
function leTrainPeutIlBouger(le_train, plateau, contexte){
	...
	if(le_train.tableau.length >= 1){
		...
		if(tete_du_train.x < LARGEUR_PLATEAU - 1) {
			if((Type_de_train.Locomotive === plateau.cases[tete_du_train.x+1][tete_du_train.y] || 
		...
		if(tete_du_train.y > 0){
			if((Type_de_train.Locomotive === plateau.cases[tete_du_train.x][tete_du_train.y-1] || 
		...
		if(plateau.cases[colone_case][ligne_case] === Type_de_case.Rail_horizontal){
			...
			switch (dernier_train_clique) {
				...
				case Type_de_train.Train1 : // Si on avait clique sur le boutton ajour locomotive seule L
					placerVoitureSurPlateau(plateau, ligne_case, colone_case, contexte, Type_de_train.Locomotive);
